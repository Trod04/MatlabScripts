function [ validStdTest, resultsStdPaths, resultsPksPaths,  maxSTDdev, maxPeaks ] = stdSNR (fidLog, CT, posCSV, LoggedData)
...
try
    ...
    SNRPaths = LoggedData(:,posCSV.SNR:(posCSV.SNR + (numPaths*2) - 1));
    [numRows, ~] = size(SNRPaths);
    maxSTDdev = CT.SNR(5);
    peakDef = CT.SNR(6);
    maxPeaks = CT.SNR(7);
    ...
    for i = 1 : (numPaths*2)
        averagePath(1,i) = mean(SNRPaths(:,i));
    end
    ...
    for i = 1: (numPaths*2)
        for j = 1: numRows
            devVals(j,i) = abs(SNRPaths(j,i) -averagePath(1,i));
        end
    end
